...
- Editar `log-arquitectura.gs` exige **re-publicar una NUEVA implementación** del Web App (Implementar → Administrar implementaciones → Editar → Nueva versión). La `GSHEET_URL` no cambia si se actualiza la implementación existente.
...
**Files:**
- Modify: `log-arquitectura.gs` (bloque `action === 'loginGoogle'` dentro de `doGet`, y nueva función a nivel de archivo)
...
En `log-arquitectura.gs`, dentro de `if (... action === 'loginGoogle')`, justo después de `var ss = SpreadsheetApp.getActiveSpreadsheet();`, insertar:
...
En `log-arquitectura.gs`, a nivel de archivo (por ejemplo, arriba de `function autoCrearCarpeta(e)`), agregar:
...
En `log-arquitectura.gs`, a nivel de archivo (junto a `registrarParticipante`), agregar:
